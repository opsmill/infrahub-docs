---
title: Restore Infrahub on Kubernetes
---
...
To refresh a staging or demo deployment from another environment's backups on a recurring schedule instead, see [Refresh an environment on a schedule](./kubernetes-scheduled-restore.mdx).
...
## High availability (CloudNativePG)

If your deployment uses a CloudNativePG HA PostgreSQL cluster, the restore process automatically detects and targets the primary pod. No special configuration is needed. See the [Kubernetes backup guide](./kubernetes-backup.mdx#high-availability-cloudnativepg) for details on supported HA operators.
...
- [Refresh an environment on a schedule](./kubernetes-scheduled-restore.mdx)
