---
title: Back up Infrahub on Kubernetes
---
...
A scheduled backup with no retention policy grows its bucket until something fails. The chart has no retention values key, so the rules go through `backup.options.extraArgs`. See [Attach retention to scheduled backups](./retention.mdx#step-3-attach-retention-to-scheduled-backups) for the values and the delete permissions the credentials need.
...
For testing or basic setups, store backups locally in the pod:
