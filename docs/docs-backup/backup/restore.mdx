...
On Kubernetes, the `infrahub-backup` Helm chart renders the same refresh as a CronJob. See [Refresh an environment on a schedule](./kubernetes-scheduled-restore.mdx).
