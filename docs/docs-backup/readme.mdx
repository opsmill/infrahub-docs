...
- **Docker Compose**: HA setups require stopping the `task-manager` and `task-manager-background-svc` containers before running a restore to prevent database accesses during the process. See [Restore from a backup](./guides/restore-backup.mdx#docker-high-availability-deployments) for details.
