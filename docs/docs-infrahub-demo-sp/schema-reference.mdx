...
| `customer_asn` | `RoutingAutonomousSystem` | one | Optional | The customer AS used on every PE-CE eBGP session of this VPN | Generator allocates from `customer_asn_pool` |
...
| `bgp_peer_asn` | Number | Optional | Per-site override for the PE-CE peer AS. Leave unset to use `customer_asn` on the VPN | User |
...
| `pe_interface` | `InterfacePhysical` | one | Optional | PE-facing physical interface | Generator: reuses a port already described `L3VPN <vpn name>`, else allocates a free one |
| `ce_device` | `DcimDevice` | one | Optional | Pre-provisioned CE router this site terminates on. Unset = unmanaged CE | User |
| `ce_interface` | `InterfacePhysical` | one | Optional | CE port facing the PE | User |
...
The demo bootstrap sets `role = pe` on every backbone PE — 8 on the default
**financial** dataset (`pe-01`…`pe-08`), 4 on the **isp** dataset.
...
| `customer_asn_pool` | `CoreNumberPool` | Customer AS numbers for PE-CE eBGP — one per L3VPN | Integer (range 65100–65199; the backbone AS 65000 sits outside it) |
...
| `ServiceL3Vpn` | `vrf`, `customer_asn`, `status = active` | Generator |
| `ServiceL3VpnSite` | `name`, `pe_device`, `routing_protocol`, `customer_subnet`, `l3vpn` | User / catalog |
| `ServiceL3VpnSite` | `bgp_peer_asn`, `ce_device`, `ce_interface` | User / catalog (all optional) |
...
| `RoutingAutonomousSystem` (customer) | all — ASN from `customer_asn_pool` | Generator |
| `RoutingBGPSession` (PE-CE and CE-PE) | all (only when `routing_protocol = ebgp`) | Generator |
