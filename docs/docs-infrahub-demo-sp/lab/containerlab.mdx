...
|---|---|---|---|
| Arista EOS | `arista_eos` | `ceos-image` (see [Architecture](#architecture)) | Full configuration push via `invoke lab.push-arista` |
...
containerlab drives the host kernel directly (network namespaces, veth pairs,
bridges), so the lab needs a **Linux host**. On macOS or Windows, run it inside
a Linux VM — OrbStack, Lima, UTM or a cloud instance — not against Docker
Desktop from the host side.

The **financial** dataset is the demanding one: 12 Arista cEOS routers and 4
customer hosts, alongside the Infrahub stack itself.
...
cEOS-lab images are published per architecture, and the OpsMill mirror carries
no multi-arch tag, so the image has to match the host CPU:

| Host `uname -m` | Image `invoke lab.deploy` selects |
|---|---|
| `x86_64` | `registry.opsmill.io/external/ceos-image:4.36.0.1F` |
| `aarch64` / `arm64` | `registry.opsmill.io/external/ceos-image:4.33.2F` |

`invoke lab.deploy` picks the right one from the host architecture and passes it
to containerlab as `CEOS_IMAGE`; the rendered topology reads
`${CEOS_IMAGE:=<amd64 default>}`, so the artifact Infrahub renders is
host-independent. Override `CEOS_IMAGE` yourself to pin a different build.

Do not pin a cEOS-lab image older than **4.32.0F**. Earlier builds require a
cgroups v1 host and never finish booting on a cgroups v2 one — the default on
Ubuntu 21.04+, OrbStack and most current distributions — and the failure is silent.
See
[Troubleshooting](../troubleshooting.mdx#containerlab-deploy-hangs-at-running-postdeploy-actions).

### Software

- containerlab >= 0.50
- Docker, with network access to pull:
  - `registry.opsmill.io/external/ceos-image` (public mirror — no Arista
    account or manual import needed)
  - `ghcr.io/nokia/srlinux` (isp dataset only)
  - `nicolaka/netshoot` (the `cust-*` customer hosts on the CE LAN side)
...
2. **cEOS** — the OpsMill mirror
   (`registry.opsmill.io/external/ceos-image`) is public and needs no login, so
   a pull failure here is usually the wrong tag for your architecture rather
   than an access problem: only `4.33.2F` is arm64, every other tag is amd64.
   Check what you asked for:

   ```bash
   uname -m
   echo "${CEOS_IMAGE:-<unset, template default applies>}"
   ```

   To use your own Arista-supplied build instead, import it and point
   `CEOS_IMAGE` at it:
