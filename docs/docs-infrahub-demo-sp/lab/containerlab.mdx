...
2. **cEOS** — the OpsMill mirror
   ...
   a pull failure here is usually a tag that does not exist rather than an
   access problem. Note that the mirror's registry metadata is not a reliable
   guide to architecture: at least one tag declares `arm64` while carrying an
   x86 image, and pulling it on Apple Silicon succeeds and then fails to boot.
