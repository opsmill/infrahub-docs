...
- **Resource pools** — `vpn_id_pool`, `sdwan_id_pool`, `customer_asn_pool`,
  `pe_loopback_pool`, `backbone_p2p_pool`, `pe_ce_pool` for deterministic
  allocation.
...
backbone — eight Arista cEOS PEs (`pe-01`…`pe-08`) in a partial mesh, with four
pre-provisioned Arista cEOS CE routers hanging off the two hub PEs — plus three
seeded customer services — two L3VPNs and one SD-WAN — wired up end to end:
...
flowchart TB
    ...
    subgraph l3vpn["L3VPN: trading-floor-vpn — tenant: markets-trading"]
        direction LR
        ce_trading_lon["ce-trading-lon · London<br/>Arista cEOS<br/>10.200.10.0/24"]
        ce_trading_zrh["ce-trading-zrh · Zurich<br/>Arista cEOS<br/>10.200.20.0/24"]
    end

    subgraph ibvpn["L3VPN: ib-advisory-vpn — tenant: investment-banking"]
        direction LR
        ce_ib_lon["ce-ib-lon · London<br/>Arista cEOS<br/>10.200.30.0/24"]
        ce_ib_zrh["ce-ib-zrh · Zurich<br/>Arista cEOS<br/>10.200.40.0/24"]
    end

    pe01 -. "VRF · eBGP" .-> ce_trading_lon
    pe08 -. "VRF · eBGP" .-> ce_trading_zrh
    pe01 -. "VRF · eBGP" .-> ce_ib_lon
    pe08 -. "VRF · eBGP" .-> ce_ib_zrh
    ...
    class ce_trading_lon,ce_trading_zrh,ce_ib_lon,ce_ib_zrh ce
    class hub_lon,sp_fra,sp_ams edge
```

**Backbone** — Eight PE routers, all Arista cEOS (`pe-01`…`pe-08`), in eight
EMEA sites. The topology is a partial mesh of 15 p2p links: `pe-01` (London)
and `pe-08` (Zurich) are the degree-2 hub PEs carrying every customer
attachment, and `pe-02`…`pe-07` form the meshed core between them at degree
4–5. Over the top runs a full iBGP/VPNv4 mesh, ISIS for underlay reachability,
and LDP for label distribution. (The **isp** dataset instead ships a four-PE,
one-per-vendor backbone — see below.)

**Customer edge** — Four pre-provisioned Arista cEOS CE routers, two per
customer, on the hub PEs: `ce-trading-lon`/`ce-ib-lon` on `pe-01` and
`ce-trading-zrh`/`ce-ib-zrh` on `pe-08`. Each has its own rendered configuration
artifact, so the PE-CE eBGP session comes up for real in the lab.

**L3VPN services** — `trading-floor-vpn` (tenant `markets-trading`) and
`ib-advisory-vpn` (tenant `investment-banking`), each with a London and a
Zurich site, eBGP PE-CE sessions, and one /24 customer subnet per site in the
`10.200.0.0/16` supernet. Each customer peers from its own AS, allocated from
`customer_asn_pool`.
...
generator. The L3VPN generator allocates a VRF, route targets, a customer AS
from `customer_asn_pool`, a /30 from `pe_ce_pool`, sets up the PE interface,
and writes the eBGP session on both ends of each site — the CE end too, when
the site names a pre-provisioned CE router. The SD-WAN generator materialises one CPE per site, allocates the LAN
...
A single schema drives seven Jinja2 templates: Arista EOS (PE), Arista EOS
(CE), Cisco IOS-XR, Juniper Junos, Nokia SR OS / SR Linux, Cisco Viptela cEdge
(IOS-XE SD-WAN), and Versa VOS (FlexVNF). Each PE, CE, or edge ends up with a
`text/plain` configuration artifact keyed by `name__value`, ready to feed into a
CI/CD push. The default **financial** backbone is all-Arista, so it exercises
the two Arista templates; the **isp** dataset, with one PE per vendor,
exercises the full per-vendor set.
...
addressing, plus every pre-provisioned CE and its PE-CE access link. For the
default **financial** backbone that's eight Arista cEOS PEs, 15 backbone links,
and four cEOS CEs that bring up real eBGP sessions against their PEs; for the
**isp** dataset it's the Arista and Nokia PEs (Nokia SR OS substituted to SR
Linux for the lab image). See the
