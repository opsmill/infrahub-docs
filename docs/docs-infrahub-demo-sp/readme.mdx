...
backbone with two seeded customer services — one L3VPN, one SD-WAN — wired up
end to end:
...
**Backbone** — Four PE routers, one per vendor, full-mesh iBGP/VPNv4, ISIS for
underlay reachability, LDP for label distribution.

**L3VPN service (`trading-floor-vpn`)** — Tenant `markets-trading`, four sites
(one per PE) with eBGP PE-CE sessions and one /24 customer subnet per site in
...
An alternate **isp** dataset (Lumina Networks pan-European ISP, eight customer
tenants, a different default L3VPN and SD-WAN) is available via the
`INFRAHUB_DATASET` env var.
...
keyed by `name__value`, ready to feed into a CI/CD push.
...
The `clab-mpls-topology` artifact emits a containerlab YAML wiring the four
PEs (with Nokia SR OS substituted to SR Linux for the lab image). See the
