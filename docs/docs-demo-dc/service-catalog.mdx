...
Network devices and hypervisor hosts both appear here. A host shows in the
hypervisor colour and at its real height, so a 2U Dell PowerEdge R760 occupies
two units while a 1U R650 occupies one. Hosts are placed by the
`cable_virtualization_host` generator rather than by hand, so they appear only
after the virtualization data has loaded and that generator has run. See the
[virtual machine management tutorial](./virtualization.mdx) for how placement works.
