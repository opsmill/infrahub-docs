...
| `schemas/otn_devices.yml` | The <span data-fact="device-kinds">eight</span> device kinds |
...
Devices opens on the generic kind, so one click gives all <span data-fact="device-kinds">eight</span> device kinds.
