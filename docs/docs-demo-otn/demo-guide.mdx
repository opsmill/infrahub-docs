...
<span data-fact="scenarios">Nine</span> scenarios against the loaded GÉANT dataset. Every number on the three
...
| 6 | Where do the cheap pluggables reach? | The cFEC parts nowhere, the oFEC parts almost everywhere. |
...
[network map](./network-map.mdx) and then runs the same nine scenarios in
