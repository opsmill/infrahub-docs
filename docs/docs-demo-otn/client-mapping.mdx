...
The repository ships <span data-fact="checks">fourteen</span> checks: `channel_collision`,
`osnr_margin`, `units_import`, `container_capacity`, `diversity`, `provisionable`,
`channel_count_consistency`, `monitor_completeness`, `carrier_termination`,
`mux_channel_binding`, `attenuator_range`, `transceiver_placement`,
`transceiver_mode_support` and `connector_polish`. `container_capacity` is
