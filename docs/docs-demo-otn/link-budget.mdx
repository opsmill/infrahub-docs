...
`OtnOduSwitch` is the kind. Its `carriers` relationship names the wavelengths it
terminates, and `src/infrahub_demo_otn/chains.py` reads that to decide whether a
junction is real. The two wavelengths must meet at a site, that site must host
the device, and the device must terminate both.
