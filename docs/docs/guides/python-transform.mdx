...
The next step is to create the actual Python transform. The transform is a Python class that inherits from InfrahubTransform from the [Python SDK](../python-sdk). Create a file called `device_config.py`:
...
The query part refers to the file device_config.gql that we created earlier.
