...
Schemas to be loaded as part of an external repository can be defined in file(s) as described in the [Schema topic](../topics/schema). The schemas must also be explicitly identified in the `.infrahub.yml` file under `schemas`.
...
Objects to be loaded as part of an external repository can be defined in file(s) as described in the [objects topic]($(base_url)python-sdk/topics/object_file). The objects must also be explicitly identified in the `.infrahub.yml` file under `objects`.
...
Python Transformations can be defined as described in the [Transformations topic](../topics/transformation#transformpython-python-plugin). To load Python Transformations from an external repository, you must explicitly define them in the `.infrahub.yml` configuration file. The definition in `.infrahub.yml` includes the following
...
Artifact Definitions can be created as described in the [Artifacts topic](../topics/artifact). To load Artifact Definitions from an external repository, you must explicitly define them in the `.infrahub.yml` configuration file. Each Artifact Definition in `.infrahub.yml` must include the following:
...
Generator Definitions can be created as described in the [Generators topic](../topics/generator). To load Generator Definitions from an external repository, you must explicitly define them in the `.infrahub.yml` configuration file. Each Generator Definition in `.infrahub.yml` must include the following:
