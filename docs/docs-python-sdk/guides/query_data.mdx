...
By default, the [meta data or properties]($(local_base_url_2)topics/metadata) of attributes and relationships are not included. We can include these properties using the `property` argument of the SDK client's `all`, `filters` or `get` method.
