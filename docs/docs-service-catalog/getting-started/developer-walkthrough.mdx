...
- `circuit` (optional): Defines circuits with endpoints for site-to-site connections. This schema is available in `schemas/circuit/` but is not loaded by default.
...
```mermaid
flowchart LR
    classDef semaphore fill:#e6f3ff,stroke:#4a90e2,stroke-width:2px
    classDef ansible fill:#fff2e6,stroke:#ff9800,stroke-width:2px
    classDef infrahub fill:#e6ffe6,stroke:#4caf50,stroke-width:2px
    classDef device fill:#ffe6f2,stroke:#e91e63,stroke-width:2px

    S[Semaphore UI]:::semaphore
    P[deploy.yml Playbook]:::ansible
    I[Dynamic Inventory]:::ansible
    INF[Infrahub API]:::infrahub
    D1[Device 1]:::device
    D2[Device 2]:::device
    D3[Device N]:::device
    ...
    S -->|Triggers| P
    P -->|Uses| I
    I -->|Queries devices| INF
    P -->|Fetches artifacts| INF
    P -->|Deploys config| D1
    P -->|Deploys config| D2
    P -->|Deploys config| D3
```
...
This means any device added or modified in Infrahub is automatically reflected in the Ansible inventory — no manual synchronization required.
